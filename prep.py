...
def generate_data(start, end):
    ...
    for i in range(start, end):
        ...
        if data_i.any() and label_i.any():
            ...
            data_i = np.pad(data_i, ((0,), (16,), (16,)), 'constant', constant_values=(0,))
            # label_i = label_i.reshape((155, 240, 240, 1))
            for q in range(0, data_i.shape[0]):
                for x in range(16, 256):
                    for y in range(16, 256):
                        if data_i[q][x][y] == 0:
                            continue
                        print('Patch:', q, ',', x, ',', y, 'on', i)
                        if i < 200:
                            data = np.append(data, data_i[q][x - 16:x + 16, y - 16:y + 16])
                            label = np.append(label, label_i[q][x - 16, y - 16])
                        else:
